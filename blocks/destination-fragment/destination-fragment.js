...
export default function decorate(block) {
  ...
  fetch(`${aem}/graphql/execute.json/qatar-airways/get-arrivingIn-details-by-slug;slug=${slugID.textContent}`)
    .then(response => response.json())
    .then(response => {
      const { mainImage, cityTitle, description, touristAttractions, activities, travelConsiderations } = response.data.arrivingInList.items[0];
      const imageURL = `${aem}${mainImage._dynamicUrl}`;

      destinationDiv.innerHTML = `
        <div class='destination-image'>
          <img src="${imageURL}" alt="${cityTitle}">
        </div>
        <div class='destination-content'>
          <div class='destination-content-title'><h3>${cityTitle}</h3></div>
          <div class='destination-content-description'>${description.html}</div>
          <div class='destination-content-tourist'>${touristAttractions.html}</div>
          <div class='destination-content-activities'>${activities.html}</div>
          <div class='destination-content-travel'>${travelConsiderations.html}</div>                    
        </div>
      `;
    })
    .catch(error => {
